...
def read_images(dir_path: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for i in files_list:
        ...
        data_array_temp = data_array_temp_org.ravel()
    ...
    label_array = np.array(label_list, ndmin=1).T   
...
def dataset_split(data_array: np.ndarray, label_array: np.ndarray, split_ratios:List[float]
    ...
    X_test: np.ndarray of shape (# of images * split_ratios[2] x 19200 )
     ...
     Y_train: np.ndarray 1d column vector (# of images * split_ratios[0], )
        The training labels
    
    Y_dev: np.ndarray 1d column vector (# of images * split_ratios[1],  )
        The labels for the development set
    
    Y_test: np.ndarray 1d column vector (# of images * split_ratios[2],  )
